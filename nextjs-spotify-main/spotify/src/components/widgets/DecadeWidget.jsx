            ...
            <div className="flex-1 grid grid-cols-4 gap-2 content-start">
                {decades.map((decade) => (
                    <button
                        ...
                        className={`text-sm px-2 py-3 rounded-lg transition-all ${
                            isSelected(decade)
            ...
            <div className="pt-2 border-t border-zinc-700 min-h-[32px]">
                {selectedDecades.length > 0 ? (
                    <p className="text-zinc-400 text-xs truncate">
                        {selectedDecades.map(d => d.label).join(', ')}
                    </p>
                ) : (
                    <p className="text-zinc-500 text-xs">Selecciona una o más décadas</p>
                )}
            </div>
        </div>
    );
